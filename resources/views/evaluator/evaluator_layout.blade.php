...
<!-- Profile Dropdown (Accessing Evaluator Data) -->
<div class="dropdown">
  <div class="dropdown-toggle d-flex align-items-center px-3 py-2 rounded-3"
       data-bs-toggle="dropdown" role="button" aria-expanded="false" style="cursor: pointer; background-color: #f5f7fa;">
    <img src="{{ asset('img/' . (session('profile') ?? 'user_prof.png')) }}" alt="Profile"
         class="rounded-circle" style="width: 38px; height: 38px; object-fit: cover; border: 1px solid #ccc;">
    <div class="d-flex flex-column justify-content-center text-start ms-2">
      <strong class="text-dark" style="font-size: 15px;">{{ session('evaluatorFirstName') }} {{ session('evaluatorLastName') }}</strong>
  ...
  <aside id="sidebar" class="sidebar">
    <div>
      <ul class="nav flex-column">
        <li class="nav-item">
          <a href="{{ route('evaluator.dashboard') }}" class="nav-link {{ Route::is('evaluator.dashboard') ? 'active' : '' }}">
            <i class="fas fa-home me-2"></i> <span>Dashboard</span>
          </a>
        </li>
        <li class="nav-item">
            <a href="{{ route('evaluator.rev_doc') }}" class="nav-link {{ Route::is('evaluator.rev_doc') ? 'active' : '' }}">
            <i class="fas fa-graduation-cap me-2"></i> <span>Document</span>
            </a>
        </li>
      </ul>
    </div>
    ...
    <div class="modal-content text-center p-4 rounded-4">
      <div class="modal-body">
        ...
        <div class="d-flex justify-content-center">
          <!-- Cancel Button -->
          <button type="button" class="btn btn-outline-secondary btn-md me-2" data-bs-dismiss="modal">
            Cancel
          </button>
          ...
          <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit" class="btn btn-md text-white" style="background-color: #0C2340;">
              Logout
            </button>
          </form>
        </div>
