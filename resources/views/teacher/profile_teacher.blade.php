  ...
  <div class="row justify-content-center align-items-start">
    ...
    <div class="col-md-8 bg-light p-4 rounded shadow-sm">
      ...
      <div class="mb-3">
        ...
        <input type="text" class="form-control" value="{{ session('Address') }}" disabled>
      </div>
      ...
      <div class="modal-body row justify-content-center">
        ...
        <div class="col-md-7">
          ...
          <div class="mb-3">
            ...
            <select name="province_id" id="provinceDropdown" class="form-select" required>
          ...
          <div class="mb-3">
            ...
            <select name="municipality_id" id="municipalityDropdown" class="form-select" required>
          ...
          <div class="mb-3">
            ...
            <select name="barangay_id" id="barangayDropdown" class="form-select" required>
...
<script>
 ...
 document.getElementById('provinceDropdown').addEventListener('change', function () {
  ...
  fetch(`/municipalities/${provinceId}`)
    .then(res => res.json())
    .then(data => {
      const muniSelect = document.getElementById('municipalityDropdown');
      muniSelect.innerHTML = '<option value="">-- Select Municipality --</option>';
      data.forEach(item => {
        muniSelect.innerHTML += `<option value="${item.id}">${item.name}</option>`;
      });
      document.getElementById('barangayDropdown').innerHTML = '<option value="">-- Select Barangay --</option>';
    });
});

document.getElementById('municipalityDropdown').addEventListener('change', function () {
  const muniId = this.value;
  fetch(`/barangays/${muniId}`)
    .then(res => res.json())
    .then(data => {
