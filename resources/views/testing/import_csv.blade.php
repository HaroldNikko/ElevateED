...
<div class="container py-4 d-flex flex-column justify-content-between" style="min-height: 90vh;">
  <div class="bg-white rounded-4 p-4 shadow-sm flex-grow-1">
    ...
    <!-- Upload Form -->
        <form action="{{ route('district.supervisors.import') }}" method="POST" enctype="multipart/form-data" id="csvUploadForm">
        @csrf
        
              @if(isset($district))
            <input type="hidden" name="district_id" value="{{ $district->districtID }}">
        @endif
  ...
  <div class="d-flex justify-content-between mt-4">
    <a href="{{  route('admin.manageaccount')  }}" class="btn btn-outline-primary">Cancel</a>
